...
var Library = new LibraryCls({
	"chopper": new SpawningCard("icon_of_chop", 30, 0, function(){
		new_skull(x=0, y=400, func_=skeleton_walking, 1, 80, 30);
	}),

	"archer": new SpawningCard("icon_of_bow", 120, 0, function(){
	...
	"police": new SpawningCard("icon_of_police", 300, 0, function(){
	...
	"healer": new SpawningCard("icon_of_healer",400,150,function(){
		new_skull(x=0,y=400,func_=skeleton_healer_walking_func(80,
			shoot_func=function(l_l,l_r){
		        ...
		        new_healbomb(this.x+16*this.dir,this.y-22,this.team,150*this.dir,(22/Math.max(dist,15)-4*Math.max(dist,15)/900)*150,0,8/900*(150)**2,2,randomize(60, 80),80)
	    	}
		),1,600,400)
	})
